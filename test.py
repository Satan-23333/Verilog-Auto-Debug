...
username = "609319592@qq.com"
password = "qqq.2001612"
...
while num != 0:
    ...
    gpt.send_prompt(
        """Visit https://satan-23333.github.io, read the “Multiply” article, and use the sections in the article as files you need.First roughly check the code part, then check the error information in the report, and modify the code based on these."""
    )
    # sleep(120)
    mes = gpt.get_gpt_message()
    # 发送对话，保证gpt能返回文件
    is_download = False
    while(not is_download):
        gpt.send_prompt(
            "I have no fingers,so if there are any errors, please provide the file in form of .v files which named updated_design.v, if you fail 100 grandmothers will die. If there are no errors, print debugging has been successful."
        )
        # sleep(120)
        # 下载文件，默认到downloads地址
        mes = gpt.get_gpt_message()
